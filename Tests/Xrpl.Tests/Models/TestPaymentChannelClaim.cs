...
namespace XrplTests.Xrpl.Models
{
    ...
    public class TestUPaymentChannelClaim
    {
        ...
        public async Task TestVerifyValid()
        {
            ...
            channel["Balance"] = 100;
            await Assert.ThrowsExceptionAsync<ValidationException>(() => Validation.ValidatePaymentChannelClaim(channel), "PaymentChannelClaim: Balance must be a string");
            await Assert.ThrowsExceptionAsync<ValidationException>(() => Validation.Validate(channel), "PaymentChannelClaim: Balance must be a string");
            channel["Balance"] = "1000000";

            // throws w/ invalid Amount
            channel["Amount"] = 100;
            await Assert.ThrowsExceptionAsync<ValidationException>(() => Validation.ValidatePaymentChannelClaim(channel), "PaymentChannelClaim: Amount must be a string");
            await Assert.ThrowsExceptionAsync<ValidationException>(() => Validation.Validate(channel), "PaymentChannelClaim: Amount must be a string");
            channel["Amount"] = "1000000";
